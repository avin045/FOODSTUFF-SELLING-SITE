<?php
require "./phpscripts/db.php";
// echo "connection success";


$error = "";
$error1 = "";
$error2 = "";

$name = "";
$uname = '';
$gml = '';
$email = '';
$errorname = "";

if ($_SERVER['REQUEST_METHOD'] === 'POST') {


    //mname , descri which are given name in forms so if you get data from the post you must mention that name to here to get.
    $name = $_POST['name'];
    // $name = 'gh';
    // echo $name;
    $uname = $_POST['uname'];
    // echo $uname;
    $password = $_POST['pwd'];
    //var_dump($mobile_image);
    // echo $password;
    $gml = $_POST['gmli'];
    // echo $gml;
    $email = $_POST['email'];
    // echo $email;


    $query = $pdo->prepare("SELECT * from fregister WHERE uname=?");
    $query->execute([$uname]);
    $result = $query->rowCount();
    if ($result > 0) {
        $error = "*Username is already taken";
    }


    if ($name == "") {
        $errorname = "*Username is already taken";
    }


    $query1 = $pdo->prepare("SELECT * from fregister WHERE email=?");
    $query1->execute([$email]);
    $result1 = $query1->rowCount();
    if ($result > 0) {
        $error1 = "*Email is already in use";
    }

    $query2 = $pdo->prepare("SELECT * from fregister WHERE email=?");
    $query2->execute([$gml]);
    $result2 = $query1->rowCount();
    if ($result > 0) {
        $error2 = "*Market License is already registered";
    }

    echo '<pre>';
    // var_dump($query);
    echo '</pre>';

    echo '<pre>';
    // var_dump($result);
    echo '</pre>';

    if (empty($error)) {
        $statement = $pdo->prepare("INSERT INTO fregister (name, uname, password, gml, email) 
                VALUES (:name, :uname,:password,:gml,:email)");



        $statement->bindValue(':name', $name);
        $statement->bindValue(':uname', $uname);
        $statement->bindValue(':password', $password);
        $statement->bindValue(':gml', $gml);
        $statement->bindValue(':email', $email);
        $statement->execute();


        Header('Location:farmerslogin.php');
    }
}


?>


<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />

    <!-- custom css -->
    <link rel="stylesheet" type="text/css" href="./farmers.css">

    <!-- bootstrap css -->
    <link rel="stylesheet" href="../Assets/css/bootstrap.min.css">
    <!-- Assets/css/bootstrap.min.css -->

    <!-- font awesome -->
    <script src="https:kit.fontawesome.com/deef53b5f5.js" crossorigin="anonymous"></script>
    <!-- <link rel="stylesheet" href="https:cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"> -->

    <title>For Farmers - Foodstuff</title>
</head>


<body>
    <div class="container mt-3 mb-3">
        <div class="row d-flex align-items-center justify-content-center">
            <div class="col-md-6">
                <div class="card px-5 py-5">
                    <span class="circle"><i class="fa fa-check"></i></span>
                    <h5 class="mt-3">
                        Join with Foodstuff Community<br />

                    </h5>

                    <small class="mt-2 text-muted">
                        Get register with us and produce some quality products to our users.<br>
                        <div class="text-light">
                            *Definitely you need the 11 Digit of G.M (Government Market's) License ID.
                        </div>
                    </small>
                    <form id="form" action="" method="POST">
                        <!-- ./farmerslogin.php -->

                        <div class="form-input">
                            <i class="fas fa-envelope"></i>
                            <!-- email pattern:/^[^ ]+@[^ ]+\.[a-z]{2,3}$/; -->
                            <input type="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" class="form-control text-light" placeholder="Email address" name="email" value="<?php echo $email; ?>" required />
                            <div class="text-warning" role="alert">
                                <!-- A simple warning alert—check it out! -->
                                <?php
                                echo $error1;
                                ?>
                            </div>
                        </div>

                        <div class="form-input">
                            <i class="fas fa-user-tie"></i>
                            <input type="text" class="form-control text-light" placeholder="Name" name="name" value="<?php echo $name; ?>" required />
                            <div class="text-warning" role="alert">
                                <!-- A simple warning alert—check it out! -->
                                <?php
                                echo $errorname;
                                ?>
                            </div>
                        </div>

                        <div class="form-input">
                            <i class="fa fa-user"></i>
                            <input type="text" class="form-control text-light" placeholder="User name" name="uname" onkeyup="nospaces(this)" onkeypress="return blockSpecialChar(event)" autocapitalize="off" value="<?php echo $uname; ?>" required />
                            <div class="text-warning" role="alert">
                                <!-- A simple warning alert—check it out! -->
                                <?php
                                echo $error;
                                ?>
                            </div>
                            <small id="unameerr" class="text-muted"></small>
                            <small id="unameerrr" class="text-danger pt-1"></small>
                        </div>

                        <!-- password -->


                        <div class="form-input">
                            <i class="fas fa-lock"></i>
                            <input type="password" id="password" minlength="6" maxlength="10" class="form-control text-light" placeholder="Password" name="pwd">
                            <button class="fas fa-eye eye" id="pwd" type="button" onclick="toggle(this);"></button>
                        </div>



                        <!-- retype password -->


                        <div class="form-input">
                            <!-- <i class="fas fa-lock"></i> -->
                            <i class="fas fa-keyboard"></i>
                            <input type="password" id="repassword" minlength="6" maxlength="10" class="form-control text-light" placeholder="Confirm password" onkeyup="matchPassword();">



                            <div>
                                <span class="passwordvalid" style="color: red;" id="passwordvalid"></span>
                            </div>


                        </div>
                        <div class="form-input">
                            <i class="fas fa-id-badge"></i>
                            <input type="text" class="form-control text-light" minlength="11" maxlength="11" placeholder="G.M License ID" name="gmli" value="<?php echo $gml; ?>" required />
                            <div class="text-danger" role="alert">
                                <!-- A simple warning alert—check it out! -->
                                <?php
                                echo $error2;
                                ?>
                            </div>
                        </div>
                        <!-- <input type="file" id="myFile" name="filename"> -->

                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" value="" id="flexCheckChecked" checked required />
                            <label class="form-check-label" for="flexCheckChecked">
                                I agree all the <a href="#" type="button" class="text-decoration-none" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Terms and
                                    Conditions </a>
                            </label>
                        </div>


                        <!-- Modal -->
                        <div class="modal fade" id="staticBackdrop" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content bg-dark">
                                    <div class="modal-header text-light">
                                        <h5 class="modal-title" id="exampleModalLabel">Terms and Conditions</h5>
                                        <button type="button" class="btn-close bg-white" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body text-light mb-2">
                                        Our terms is very simple, <br>
                                        it's only about what the permissions you gave to access our site and what
                                        the details you provided here to sale your products. <br><br>
                                        That's it.
                                    </div>
                                </div>
                            </div>
                        </div>



                        <!-- button for form -->
                        <button type="submit" class="btn btn-success mt-3 signup" style="display: inline-block; width: 100%;">
                            Register now
                        </button>
                    </form>


                    <div class="text-center mt-4">
                        <span>Already a member?</span>
                        <a href="./farmerslogin.php" class="text-decoration-none">Sign In</a>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- jquery -->
    <script src="../Assets/Jquery/jquery-3.6.0.min.js"></script>

    <!-- bootstrap  -->
    <script src="../Assets/js/bootstrap.min.js"></script>

    <script>
        //for hide and unhide the passwords

        function toggle(button) {
            var pass = document.getElementById("password");

            if (pass.type == "password") {
                // button.innerHTML = "";
                pass.type = "text";
                $(".eye").removeClass("fas fa-eye");
                $(".eye").toggleClass("fas fa-eye-slash");
            } else {
                // button.innerHTML = "";
                pass.type = "password";
                $(".eye").removeClass("fas fa-eye-slash");
                $(".eye").toggleClass("fas fa-eye");

            }
        }


        $("#form").submit(function() {
            if ($("#password").val() != $("#repassword").val()) {
                // alert("password should be same");
                $("#passwordvalid").html("Password cannot Matched");
                return false;
            }

            if ($("#password").val() == "") {
                // alert("password should be same");
                $("#passwordvalid").html("Password is must");
                return false;
            }
        })

        $(document).ready(() => {
            $("#repassword").keyup(passwordvalid);
        });

        function nospaces(t) {
            if (t.value.match(/\s/g)) {
                t.value = t.value.replace(/\s/g, '');
                $("#unameerr").html("*Special case automatically removed, Use' _ (underscore)' ");
                // return false;
            } else {
                $("#unameerr").html("");
            }

            if (t.value.match(/[`~!@#$%^&*|•√π÷×¶∆€¥$¢^°=<>()-+{}[\]\\|,.//?;':"]/g)) {
                t.value = t.value.replace(/[`~!@#$%^&*|•√π÷×¶∆€¥$¢^°=<>()-+{}[\]\\|,.//?;':"]/g, '');
                // @#₹&-+()/~`|•√π÷×¶∆€¥$¢^°={}\%©®™✓[]<>*;:'"()/
                $("#unameerrr").html("*Use '_'  or  '-' ");
                // return false;
            } else {
                $("#unameerrr").html("");
            }
        }

        function blockSpecialChar(e) {
            var k = e.keyCode;
            return ((k > 64 && k < 91) || (k > 96 && k < 123) || k == 8 || k == 95 || k == 32 || (k >= 48 && k <= 57));
        }
    </script>

</body>

</html>